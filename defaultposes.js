 ...
 */
export function setupInitialCameraPose(dirUrl, gCamera, gOrbitControls, gPointerLockControls) {
  let initialPoses = {
    'default': {
   ...
   * @param {number} z
   */
  function cameraLookAt(gCamera, gOrbitControls, gPointerLockControls, x, y, z) {
    if (gOrbitControls) {
  ...
  function setCameraPose(d, gCamera, gOrbitControls, gPointerLockControls) {
    gCamera.position.x = d['position'][0];
    gCamera.position.y = d['position'][1];
    gCamera.position.z = d['position'][2];
    cameraLookAt(gCamera, gOrbitControls, gPointerLockControls, d['lookat'][0], d['lookat'][1], d['lookat'][2]);
  }

  setCameraPose(initialPoses['default'], gCamera, gOrbitControls, gPointerLockControls);
  for (let sceneName in initialPoses) {
    if (dirUrl.includes(sceneName)) {
      setCameraPose(initialPoses[sceneName], gCamera, gOrbitControls, gPointerLockControls);
      break;
    }
  }
