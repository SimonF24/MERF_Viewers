 ...
 * @return {string}
 */
export function createViewDependenceFunctions(
  ...
  for (let layerIndex = 0; layerIndex < 3; layerIndex++) {
    const weights = scene_params[layerIndex + '_weights'];
    const bias = scene_params[layerIndex + '_bias'];
    const width = weights.length;
    const height = weights[0].length;
    const new_width = makeMultipleOf(width, 4);
    const new_height = makeMultipleOf(height, 4);
    let new_weights = Array.from(Array(new_width), () => new Array(new_height));
    let new_bias = Array(new_height);
