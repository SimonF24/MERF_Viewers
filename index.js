...
// import * as THREE from './third_party/three.js';
import { addHandlers, setupCameraControls, updateCameraControls } from './input.js';
import { createNetworkWeightTexture, createViewDependenceFunctions } from './viewdependency.js';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { PointerLockControls } from 'three/addons/controls/PointerLockControls.js';
import { PNG } from './third_party/png.js';
import { renderProgressively, setupProgressiveRendering } from './progressive.js';
...
function loadPNG(rgbaUrl) {
  ...
  rgbaPromise.catch(error => {
    console.error('Could not load PNG image from: ' + rgbaUrl + ', error: ' + error);
    return;
  });
  return rgbaPromise;
}
...
function createEmptyVolumeTexture(width, height, depth, format, filter) {
  ...
  } else if (format === THREE.RedFormat) {
    data = new Uint8Array(width * height * depth);
  }
  let volumeTexture = new THREE.Data3DTexture(data, width, height, depth);
 ...
 * @return {!THREE.Data3DTexture} Volume texture
 */
function createVolumeTexture(data, format, filter) {
  let volumeTexture = new THREE.Data3DTexture(data);
  volumeTexture.format = format;
  volumeTexture.generateMipmaps = false;
  volumeTexture.magFilter = volumeTexture.minFilter = filter;
  volumeTexture.wrapS = volumeTexture.wrapT = volumeTexture.wrapR =
 ...
 * @param {!THREE.Data3DTexture} texture Volume texture to upload to
 ...
 * @param {!THREE.WebGLRenderer} gRenderer WebGL renderer
 */
...
function createEmptyTriplaneTextureArray(width, height, format) {
  let texture = new THREE.DataArrayTexture(null, width, height, 3);
 ...
 */
function loadScene(
    dirUrl, width, height, useLargerStepsWhenOccluded, nearPlane, gCamera, gOrbitControls, gPointerLockControls, gRenderer) {
  ...
  filenameToLinkPromise.then(filenameToLink => {
    ...
    let viewdepdencyShaderPromise = loadTextFile('viewdependency.glsl');
    let fragmentShaderPromise = loadTextFile('fragment.glsl');
    let textPromises =
    ...
    textPromises.then(parsed => {
      const sceneParams = parsed[0];
      const viewDependenceNetworkShaderFunctions = parsed[1];
      const rayMarchFragmentShaderBody = parsed[2];
      ...
      if (useSparseGrid) {
        ...
        sparseGridRgbTexture = _createEmptyAtlasVolumeTexture(THREE.RGBAFormat);
        ...
        // let featureTextures = new Array(
        //   sceneParams['num_slices']
        //   ).fill(0).map((_, index) => {
        //     return textureLoader.load(
        //       filenameToLinkTranslator.translate(
        //         'sparse_grid_features_' + digits(index, 3) + '.png'
        //       )
        //     );
        //   });

        // sparseGridFeaturesTexture = createVolumeTexture(
        //   featureTextures, THREE.RGBAFormat, THREE.LinearFilter
        // )

        // var rgbAndDensityTextures = new Array(sceneParams['num_slices']).fill(0).map((_, index) => {
        //   return textureLoader.load(
        //     filenameToLinkTranslator.translate(
        //       'sparse_grid_rgb_and_density_' + digits(index, 3) + '.png'
        //     )
        //   );
        // });
        
        // // Clone the array of textures and set the magFilter property.
        // var rgbAndDensityTexturesClone = rgbAndDensityTextures.map(texture => {
        //   var clone = texture.clone();
        //   return clone;
        // });
        
        // // Create two Data3DTexture objects, one for RGB and one for A.
        // sparseGridRgbTexture = createVolumeTexture(
        //   rgbAndDensityTextures, THREE.RGBAFormat, THREE.LinearFilter
        // );
        // sparseGridDensityTexture = createVolumeTexture(
        //   rgbAndDensityTexturesClone, THREE.RedFormat, THREE.LinearFilter
        // );

        // // Split up the RGBA channels of each texture using a shader.
        // let shaderMaterial = new THREE.ShaderMaterial({
        //   uniforms: {
        //     textureRGB: { value: sparseGridRgbTexture },
        //     textureA: { value: sparseGridDensityTexture }
        //   },
        //   vertexShader: `
        //     varying vec2 vUv;
        //     void main() {
        ...
        //     varying vec2 vUv;
        //     void main() {
        //       vec4 colorRGB = texture2D(textureRGB, vUv);
        //       vec4 colorA = texture2D(textureA, vUv);
        //       gl_FragColor = vec4(colorRGB.rgb, colorA.a);
        //     }
        //   `
        // });

        // sparseGridRgbTexture.image.data = shaderMaterial.uniforms.textureRGB.value.data;
        // sparseGridRgbTexture.needsUpdate = true;
        // sparseGridDensityTexture.image.data = shaderMaterial.uniforms.textureA.value.data;
        // sparseGridDensityTexture.needsUpdate = true;
        ...
        // let sparseGridPromises = [];
        // let sparseGridRgbAndDensityPromise, sparseGridFeaturesPromise;
        // let sparseGridRgbAndDensityUrl, sparseGridFeaturesUrl;
        // for (let i = 0; i < sceneParams['num_slices']; i++) {
        //   sparseGridRgbAndDensityUrl = filenameToLinkTranslator.translate(
        //     'sparse_grid_rgb_and_density_' + digits(i, 3) + '.png'
        //   );
        //   sparseGridRgbAndDensityPromise = loadPNG(sparseGridRgbAndDensityUrl);
        //   sparseGridRgbAndDensityPromise.then(data => {
        //     onImageLoaded();
        //     return data;
        //   });
        //   sparseGridFeaturesUrl = filenameToLinkTranslator.translate(
        //     'sparse_grid_features_' + digits(i, 3) + '.png'
        //   );
        //   sparseGridFeaturesPromise = loadPNG(sparseGridFeaturesUrl);
        //   sparseGridFeaturesPromise.then(data => {
        //     onImageLoaded();
        //     return data;
        //   });
        //   sparseGridPromises.push(
        //     sparseGridRgbAndDensityPromise,
        //     sparseGridFeaturesPromise
        //   );
        // }
        // sparseGridPromises = Promise.all(sparseGridPromises);
        // updateLoadingProgress();

        // sparseGridRgbTexture = createEmptyVolumeTexture(
        //   sceneParams['atlas_width'], sceneParams['atlas_height'],
        //   sceneParams['atlas_depth'], THREE.RGBAFormat, THREE.LinearFilter
        // );
        // sparseGridDensityTexture = createEmptyVolumeTexture(
        //   sceneParams['atlas_width'], sceneParams['atlas_height'],
        //   sceneParams['atlas_depth'], THREE.RedFormat, THREE.LinearFilter
        // );
        // sparseGridFeaturesTexture = createEmptyVolumeTexture(
        //   sceneParams['atlas_width'], sceneParams['atlas_height'],
        //   sceneParams['atlas_depth'], THREE.RGBAFormat, THREE.LinearFilter
        // );
        // sparseGridPromises.then(sparseGridData => {
        //   let sparseGridRgbStack = new Uint8Array(
        //       sceneParams['atlas_width'] * sceneParams['atlas_height']
        //       * sceneParams['atlas_depth'] * 3);
        //   let sparseGridDensityStack = new Uint8Array(
        //       sceneParams['atlas_width'] * sceneParams['atlas_height']
        //       * sceneParams['atlas_depth']);
        //   let sparseGridFeatureStack = new Uint8Array(
        //       sceneParams['atlas_width'] * sceneParams['atlas_height']
        //       * sceneParams['atlas_depth'] * 4);
        //   // sparseGridRgbTexture.image.data =
        //   //     sparseGridRgbStack;
        //   // sparseGridDensityTexture.image.data =
        //   //     sparseGridDensityStack;
        //   // sparseGridFeaturesTexture.image.data =
        //   //     sparseGridFeatureStack;
        //   let i = 0;
        //   let j = 0;
        //   let quo, rem;
        //   for (let z = 0; z < sceneParams['atlas_depth'] * 3; z++) {
        //     for (let y = 0; y < sceneParams['atlas_height']; y++) {
        //       for (let x = 0; x < sceneParams['atlas_width']; x++) {
        //         quo = Math.floor(z / 3);
        //         rem = z % 3;
        //         sparseGridRgbStack[i] =
        //             sparseGridData[quo * 2][z * y * x * 4 + j];
        //         if (rem == 0) {
        //           sparseGridDensityStack[i] =
        //               sparseGridData[quo * 2][z * y * x];
        //           sparseGridFeatureStack[i] = 
        //               sparseGridData[quo * 2 + 1][z * y * x];
        //           i++;
        //         }
        //         j++;
        //       }
        ...
        //   }
        //   // Update the texture.
        //   sparseGridRgbTexture.image.data = sparseGridRgbStack;
        //   sparseGridRgbTexture.needsUpdate = true;
        //   sparseGridDensityTexture.image.data = sparseGridDensityStack;
        //   sparseGridDensityTexture.needsUpdate = true;
        //   sparseGridFeaturesTexture.image.data = sparseGridFeatureStack;
        //   sparseGridFeaturesTexture.needsUpdate = true;
        ...
        sparseGridBlockIndicesTexture = createEmptyVolumeTexture(
          v, v, v, THREE.RGBAFormat, THREE.NearestFilter);
        sparseGridBlockIndicesPromise.then(sparseGridBlockIndicesImage => {
          ...
          let sparseGridBlockIndicesRgba =
              new Uint8Array(v * v * v * 4);
      ...
      requestAnimationFrame(
          t => update(
              t, dirUrl, filenameToLinkTranslator, useSparseGrid, sceneParams, gCamera, gOrbitControls, gPointerLockControls, gRayMarchScene, gRenderer));
    });
  });
}
...
function initFromParameters() {
  ...
  let useLargerStepsWhenOccluded = quality != 'high';
  ...
  let gCamera = new THREE.PerspectiveCamera(
      ...
      nearPlane, 100.0);
  gCamera.fov = vfovy;
  setupProgressiveRendering(gCamera, view, lowResFactor);
  gRenderer.autoClear = false;
  gRenderer.setSize(view.offsetWidth, view.offsetHeight);

  const mouseMode = params.get('mouseMode');
  [gOrbitControls, gPointerLockControls] = setupCameraControls(gCamera, gOrbitControls, gPointerLockControls, mouseMode, view);
  setupInitialCameraPose(dirUrl, gCamera, gOrbitControls, gPointerLockControls);
  loadScene(
      dirUrl, Math.trunc(view.offsetWidth / lowResFactor),
      Math.trunc(view.offsetHeight / lowResFactor), useLargerStepsWhenOccluded,
      nearPlane, gCamera, gOrbitControls, gPointerLockControls, gRenderer);
}
...
function loadOnFirstFrame(
    dirUrl, filenameToLinkTranslator, useSparseGrid, sceneParams, gRayMarchScene, gRenderer) {
  ...
  if (useSparseGrid) {
    ...
    let sparseGridDensityTexture = _getUniform('sparseGridDensity');
    let sparseGridRgbTexture = _getUniform('sparseGridRgb');
    let sparseGridFeaturesTexture = _getUniform('sparseGridFeatures');
    ...
    let uploadFn =
        (rgbaImage, sliceIndex, volumeWidth, volumeHeight, sliceDepth) => {
          ...
          let rgbPixels =
              new Uint8Array(volumeWidth * volumeHeight * sliceDepth * 4);
          let densityPixels =
          ...
          uploadVolumeSlice(
              ...
              volumeHeight, sliceDepth, gRenderer);
          uploadVolumeSlice(
    ...
    uploadFn =
        (rgbaImage, sliceIndex, volumeWidth, volumeHeight, sliceDepth) => {
          console.log(sparseGridFeaturesTexture)
          uploadVolumeSlice(
              rgbaImage, sparseGridFeaturesTexture, sliceIndex, volumeWidth,
              volumeHeight, sliceDepth, gRenderer);
        };
    let sparseGridFeaturesTexturePromise =
...
function update(
  ...
  loadOnFirstFrame(
      dirUrl, filenameToLinkTranslator, useSparseGrid, sceneParams, gRayMarchScene, gRenderer);
  updateCameraControls(gCamera, gOrbitControls, gPointerLockControls);
  renderProgressively(gCamera, gDisplayMode, gRayMarchScene, gRenderer, gStepMult);
  gStats.update();
  requestAnimationFrame(
      t => update(
        t, dirUrl, filenameToLinkTranslator, useSparseGrid, sceneParams, gCamera, gOrbitControls, gPointerLockControls, gRayMarchScene, gRenderer));
